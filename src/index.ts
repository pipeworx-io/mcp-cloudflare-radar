 ...
 * - radar_domain_rank:  a single domain's popularity rank + trajectory
 * - radar_top_domains:  the most-visited domains globally or by location
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Cloudflare Radar');
}
...
const tools: McpToolExport['tools'] = [
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        domain: { type: 'string', description: 'Domain to look up, e.g. "reddit.com" (no scheme/path).' },
        date: { type: 'string', description: 'ISO date (YYYY-MM-DD) for a historical ranking snapshot. Omit for the latest.' },
      },
      required: ['domain'],
    },
  },
  {
    name: 'radar_top_domains',
    description:
    ...
    inputSchema: {
      ...
      properties: {
        ...
        location: { type: 'string', description: '2-letter location code (e.g., "US", "DE", "JP"). Omit for global.' },
        date: { type: 'string', description: 'ISO date (YYYY-MM-DD) for a historical top-list. Omit for the latest.' },
      },
      required: [],
    },
  },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  switch (name) {
    ...
    case 'radar_domain_rank':
      return radarDomainRank(apiKey, args);
    case 'radar_top_domains':
      return radarTopDomains(apiKey, args);
    default:
...
function commonParams(args: Record<string, unknown>): URLSearchParams {
  ...
  params.set('dateRange', String(args.date_range ?? '28d'));
  if (args.location) params.set('location', String(args.location).toUpperCase());
  return params;
}

interface IqiSummaryResp {
  result?: {
...
async function fetchIqiMetric(
  apiKey: string,
  metric: 'bandwidth' | 'dns' | 'latency',
  baseParams: URLSearchParams,
): Promise<IqiSummaryResp> {
  const params = new URLSearchParams(baseParams);
  params.set('metric', metric);
  return radarFetch<IqiSummaryResp>(apiKey, '/quality/iqi/summary', params);
}

async function internetQuality(apiKey: string, args: Record<string, unknown>) {
  // The IQI summary endpoint returns p25/p50/p75 for ONE metric at a time.
  // We fetch all three in parallel and merge so the agent gets a single
  // tidy view of bandwidth (Mbps) + DNS resolution time + latency.
  const base = commonParams(args);
  // Per-metric graceful: if one of the three IQI subcalls fails (Radar
  // hiccup, scope issue on one metric), fall back to an empty response
  // for that branch. Downstream optional-chaining produces null values
  // and the agent still sees the metrics that succeeded.
  const fallback: IqiSummaryResp = {};
  const [bw, dns, lat] = await Promise.all([
    fetchIqiMetric(apiKey, 'bandwidth', base).catch(() => fallback),
    fetchIqiMetric(apiKey, 'dns', base).catch(() => fallback),
    fetchIqiMetric(apiKey, 'latency', base).catch(() => fallback),
  ]);

  const num = (v?: string) => (v != null ? Number(v) : null);
  const range = bw.result?.meta?.dateRange?.[0] ?? dns.result?.meta?.dateRange?.[0];
  const unit = (r: IqiSummaryResp) => r.result?.meta?.units?.[0]?.value ?? null;

  return {
    location: args.location ?? 'GLOBAL',
    date_range: args.date_range ?? '28d',
    range_start: range?.startTime ?? null,
    range_end: range?.endTime ?? null,
    bandwidth: {
      unit: unit(bw),
      p25: num(bw.result?.summary_0?.p25),
      median: num(bw.result?.summary_0?.p50),
      p75: num(bw.result?.summary_0?.p75),
    },
    dns_resolution: {
      unit: unit(dns),
      p25: num(dns.result?.summary_0?.p25),
      median: num(dns.result?.summary_0?.p50),
      p75: num(dns.result?.summary_0?.p75),
    },
    latency: {
      unit: unit(lat),
      p25: num(lat.result?.summary_0?.p25),
      median: num(lat.result?.summary_0?.p50),
      p75: num(lat.result?.summary_0?.p75),
    },
  };
}
...
async function topLocations(apiKey: string, args: Record<string, unknown>) {
  ...
  const params = new URLSearchParams();
  // Always send a window. Radar rejects a request with neither `dateRange` nor an
  // explicit start/end (400 code 2001), so omitting it when the caller omits it made
  // the no-argument call — the obvious first call, and the one the schema advertises
  // as valid via `required: []` — fail every time. The schema already documented
  // "default 28d"; only the implementation was missing it. Every other tool in this
  // pack gets this right through `commonParams`; this one built its own params.
  params.set('dateRange', String(args.date_range ?? '28d'));
...
async function radarDomainRank(apiKey: string, args: Record<string, unknown>) {
  const domain = String(args.domain ?? '').trim();
  if (!domain) throw new Error('Cloudflare Radar: "domain" is required, e.g. "reddit.com"');

  const params = new URLSearchParams();
  params.set('includeTopLocations', 'true');
  if (args.date) params.set('date', String(args.date));

  const data = await radarFetch<DomainRankResp>(
    apiKey,
    `/ranking/domain/${encodeURIComponent(domain)}`,
    params,
  );

  const details = data.result?.details_0;
  const rank = details?.rank ?? null;
  const bucket = details?.bucket ?? null;
  // Radar's floor: anything below its lowest tracked bucket comes back as an
  // open-ended ">N" string (observed ">200000") — and it comes back IDENTICALLY
  // for a real domain with modest traffic and for a domain that has never
  // existed. Radar does not verify domain existence; it just applies its
  // bucketing rule to whatever string it's given. Fleet #1140 follow-up: a
  // caller-facing `ranked: true` on that floor value asserted something the
  // endpoint never checked, which reads as a positive finding when it is
  // really "no signal at all" — worse than no field, because it looks like one.
  const isFloorBucket = bucket != null && bucket.startsWith('>');
  const hasSignal = rank != null || (bucket != null && !isFloorBucket);

  const interpretation = rank != null
    ? `Ranked #${rank} globally by Cloudflare Radar's popularity index — exact ranks are only reported inside the top 100.`
    : hasSignal
      ? `Falls in Radar's "top ${bucket}" popularity bucket: meaningfully visited, though Radar reports a bucket rather than an exact rank below its top 100.`
      : 'Cloudflare Radar has no popularity signal for this domain. This is NOT evidence the domain is real, dead, or specifically low-traffic — Radar does not verify domain existence, so a genuine-but-obscure site and a domain that has never existed both come back with this same "no signal" result.';
  ...
  return {
    ...
    bucket,
    interpretation,
    categories: (details?.categories ?? []).map((c) => ({ id: c.id ?? null, name: c.name ?? null })),
    top_locations: (details?.top_locations ?? []).map((l) => ({
...
async function radarTopDomains(apiKey: string, args: Record<string, unknown>) {
  const params = new URLSearchParams();
  ...
  const data = await radarFetch<TopDomainsResp>(apiKey, '/ranking/top', params);
